	...
	public void getMenu() throws InterruptedException {
		...
		for(int i=0; i<menu.size(); i++) {
			...
			for(int j=0; j<submenu.size();j++) {
				...
				List<WebElement> ssmenu=driver.findElements(By.xpath("//ul[@class='topnav bodytext']/li"+(j+1)+"/descendant::li[@class='sublist_item']/descendant::div/a[text()='"+wb2.getText()+"']/../following-sibling::ul/li/a/span"));
				
				for(int k=0; k<ssmenu.size(); k++) {
					Thread.sleep(2000);
					System.out.println(ssmenu.get(k).getText());
				}
			}
